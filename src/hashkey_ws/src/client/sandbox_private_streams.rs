...
#[derive(Debug)]
pub struct HashkeySandboxPrivateStreamsClient {
	base_url: String,
}

impl HashkeySandboxPrivateStreamsClient {
	/// connect to the hashkey websocket server
	pub async fn new() -> Self {
		Self {
			base_url: "wss://stream-pro.sim.hashkeydev.com/api/v1/ws/{listenKey}".to_string(),
 		}
	}
