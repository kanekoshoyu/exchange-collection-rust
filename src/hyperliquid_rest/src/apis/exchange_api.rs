...
/// struct for typed errors of method [`exchange_post`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
pub enum ExchangePostError {
...
/// This endpoint allows you to place an order on the exchange. You can choose between limit and trigger orders, set time-in-force options, and more. See the Python SDK for detailed examples of order field usage. 
pub async fn exchange_post(configuration: &configuration::Configuration, exchange_post_request: models::ExchangePostRequest) -> Result<models::ExchangePost200Response, Error<ExchangePostError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_exchange_post_request = exchange_post_request;

    let uri_str = format!("{}/exchange", configuration.base_path);
    let mut req_builder = configuration.client.request(reqwest::Method::POST, &uri_str);

    if let Some(ref user_agent) = configuration.user_agent {
        req_builder = req_builder.header(reqwest::header::USER_AGENT, user_agent.clone());
    }
    req_builder = req_builder.json(&p_exchange_post_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<ExchangePostError> = serde_json::from_str(&content).ok();
